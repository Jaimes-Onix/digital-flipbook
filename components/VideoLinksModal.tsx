    ...
    return createPortal(
        <div className="fixed inset-0 z-[220] flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-2xl rounded-[28px] shadow-2xl border flex flex-col overflow-hidden max-h-[90vh] ${bg}`}
                ...
                <div className={`flex items-center justify-between px-7 pt-6 pb-5 border-b shrink-0 ${divider}`}>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className={`text-[18px] font-bold tracking-tight ${title1}`}>
                                {readOnly ? 'Video Gallery' : (step === 'meta' ? 'Video Details' : 'Add New Video')}
                            </h3>
                            <p className={`text-[12px] font-medium ${sub}`}>{categoryName}</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {step === 'upload' && !readOnly && !isLoading && (
                        <div className="px-7 py-6 space-y-5">
                            {/* File Upload Button */}
                            <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-[24px] py-10 px-6 transition-all group bg-white/[0.02] border-white/[0.1] hover:border-lime-500/40 hover:bg-lime-500/[0.02]">
                                <div className="p-4 rounded-full bg-white/[0.05] group-hover:bg-lime-500/10 group-hover:scale-110 transition-all mb-4">
                                    <Upload size={32} className="text-zinc-600 group-hover:text-lime-400" />
                                </div>
                                <h4 className={`text-lg font-bold mb-1 ${dm ? 'text-zinc-200' : 'text-gray-900'}`}>Upload MP4 / MOV / AVI</h4>
                                <p className={`text-sm mb-6 ${dm ? 'text-zinc-500' : 'text-gray-500'}`}>High quality video storage</p>
                                <button
                                    onClick={() => fileInputRef.current?.click()}
                                    className="px-6 py-3 rounded-2xl bg-white text-zinc-900 text-sm font-bold shadow-lg hover:bg-zinc-100 transition-all active:scale-95"
                                >
                                    Choose Video File
                                </button>
                                <input
                            ...
                            <div className="flex items-center gap-4 py-2">
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-widest ${dm ? 'text-zinc-700' : 'text-gray-300'}`}>Or Add via Link</span>
                                <div className={`flex-1 h-px ${dm ? 'bg-white/5' : 'bg-gray-100'}`} />
                            </div>
                                ...
                                <>
                                    {readOnly ? (
                                        ...
                                        <>
                                            ...
                                            {/* Table header */}
                                            <div className={`grid grid-cols-[56px_1fr_1fr_130px_64px] text-[11px] font-bold uppercase tracking-[0.1em] px-3 pb-2 border-b ${divider} ${colHdr}`}>
                                                <span /><span>Name</span><span>Link / Source</span><span>Date Added</span><span className="text-right">Actions</span>
                                            </div>
                                                ...
                                                {entries.map(e => (
                                                    <div key={e.id}>
                                                        {editingId === e.id ? (
                                                            ...
                                                            <div className={`group grid grid-cols-[56px_1fr_1fr_130px_64px] items-center gap-4 px-3 py-3 rounded-2xl transition-all ${rowHov} border border-transparent hover:border-white/5`}>
                                                                <div className={`w-12 h-8 rounded-lg overflow-hidden shrink-0 flex items-center justify-center border ${dm ? 'bg-black border-white/10' : 'bg-gray-100 border-gray-200'}`}>
                                                                    {e.thumbnailUrl
                                                                        ? <img src={e.thumbnailUrl} className="w-full h-full object-cover" alt="" />
                                                                        : <Play size={12} className={dm ? 'text-zinc-700' : 'text-gray-300'} />}
                                                                </div>
                                                                {/* Name */}
                                                                <p className={`text-[14px] font-bold truncate pr-2 ${colName}`}>{e.name}</p>
                                                                {/* Link */}
                                                                <a href={e.sourceUrl} target="_blank" rel="noopener noreferrer"
                                                                    onClick={ev => ev.stopPropagation()}
                                                                    className={`text-[13px] font-medium truncate hover:underline ${dm ? 'text-lime-400/80 hover:text-lime-400' : 'text-emerald-600 hover:text-emerald-700'}`}>
                                                                    {e.sourceUrl}
                                                                </a>
                                                                {/* Date */}
                                                                <span className={`text-[12px] font-medium shrink-0 ${colDate}`}>{fmtDate(e.addedAt)}</span>
                                                                {/* Actions */}
                                                                {!readOnly && (
                                                                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                        <button onClick={() => startEdit(e)}
                                                                            className={`p-1.5 rounded-lg transition-all ${dm ? 'text-zinc-500 hover:text-lime-400 hover:bg-lime-500/10' : 'text-gray-400 hover:text-emerald-600 hover:bg-emerald-50'}`}>
                                                                            <Pencil size={14} />
                                                                        </button>
                                                                        <button onClick={() => setDeletingItem(e)}
                                                                            className={`p-1.5 rounded-lg transition-all ${dm ? 'text-zinc-500 hover:text-red-400 hover:bg-red-500/10' : 'text-gray-400 hover:text-red-500 hover:bg-red-50'}`}>
                                                                            <Trash2 size={14} />
                                                                        </button>
                                                                    </div>
                                                                )}
            ...
            {isCommitting && uploadProgress > 0 && uploadProgress < 100 && (
                <div className="absolute inset-x-0 bottom-0 z-[240] p-6 animate-in slide-in-from-bottom-4 duration-300">
                    <div className={`rounded-2xl border p-4 shadow-2xl ${dm ? 'bg-[#18181c] border-white/10 shadow-black' : 'bg-white border-gray-200 shadow-gray-200'}`}>
                        <div className="flex items-center justify-between mb-2">
                            <span className={`text-xs font-bold uppercase tracking-widest ${dm ? 'text-lime-400' : 'text-emerald-600'}`}>Uploading Video...</span>
                            <span className={`text-xs font-mono font-bold ${dm ? 'text-zinc-400' : 'text-gray-500'}`}>{Math.round(uploadProgress)}%</span>
                        </div>
                        <div className={`w-full h-2 rounded-full overflow-hidden ${dm ? 'bg-white/5' : 'bg-gray-100'}`}>
                            <div 
