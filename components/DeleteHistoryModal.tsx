...
const DeleteHistoryModal: React.FC<DeleteHistoryModalProps> = ({
    ...
    return createPortal(
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-lg max-h-[85vh] rounded-[28px] shadow-2xl border overflow-hidden animate-in zoom-in-95 fade-in duration-200 flex flex-col ${darkMode
                    ...
                    {loading ? (
                        ...
                        <div className="px-4 py-3 space-y-1.5">
                            {logs.map((log) => (
                                ...
                                >
                                    {/* Cover thumbnail */}
                                    <div className={`w-10 h-14 rounded-lg overflow-hidden flex-shrink-0 ${darkMode ? 'bg-zinc-800' : 'bg-gray-200'}`}>
                                        {log.cover_url ? (
                                            <img
                                                src={log.cover_url}
                                                alt={log.book_title}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-sm font-medium line-clamp-1 ${darkMode ? 'text-zinc-300' : 'text-gray-700'}`}>
                                            {log.book_title}
                                        </p>
                                        <div className={`flex items-center gap-2 mt-1 text-[10px] uppercase tracking-wider ${darkMode ? 'text-zinc-600' : 'text-gray-400'}`}>
                                        ...
                                        <div className={`flex items-center gap-1 mt-1 ${darkMode ? 'text-zinc-600' : 'text-gray-400'}`}>
                                            <Clock size={10} />
                                            <span className="text-[10px]">Deleted {timeAgo(log.deleted_at)}</span>
                                            <span className="text-[10px] opacity-60">· {formatDate(log.deleted_at)}</span>
                                        </div>
                                    </div>
                ...
                <div className={`flex items-center justify-end px-7 py-4 border-t flex-shrink-0 ${darkMode ? 'border-white/[0.06]' : 'border-gray-100'}`}>
                    <button
                        onClick={onClose}
                        className={`px-6 py-2.5 rounded-xl text-sm font-medium transition-colors ${darkMode
