...
import { X, Play, Video, Trash2, ExternalLink, Plus, History, Grid2X2, Pencil, Check } from 'lucide-react';
import { loadEntries, saveEntries, VideoEntry } from './VideoLinksModal';
...
const VideoGalleryModal: React.FC<Props> = ({
    ...
    useEffect(() => {
        if (isOpen) { setEntries(loadEntries(categorySlug)); setView('grid'); }
    }, [isOpen, categorySlug]);
    ...
    function confirmDelete(id: string) {
        ...
        setEntries(u); saveEntries(categorySlug, u); setDeletingItem(null);
    }
    function startEdit(e: VideoEntry) { setEditingId(e.id); setEditName(e.name); setEditUrl(e.sourceUrl); }
    function saveEdit(id: string) {
        const u = entries.map(e => e.id === id ? { ...e, name: editName.trim() || e.name, sourceUrl: editUrl.trim() || e.sourceUrl } : e);
        setEntries(u); saveEntries(categorySlug, u); setEditingId(null);
    }
    ...
    const colName = dm ? 'text-zinc-100' : 'text-gray-900';   // BLACK in light
    const colDate = dm ? 'text-zinc-400' : 'text-gray-800';   // BLACK in light
                    ...
                    {view === 'grid' && (
                        entries.length === 0 ? (
                          ...
                          ${dm ? 'bg-white/[0.05] border-white/[0.06] group-hover:shadow-black/40' : 'bg-gray-100 border-gray-100 group-hover:shadow-gray-200/80'}`}>
                                                ...
                                                <div className="absolute bottom-2 right-2 text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded-md bg-black/60 text-white">
                                                    {e.isFile ? 'FILE' : 'LINK'}
                                                </div>
                                            </div>
                                            ...
                                            <div className={`p-4 rounded-2xl border space-y-3 ${dm ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-emerald-200 bg-emerald-50'}`}>
                                                <div className="grid grid-cols-2 gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <input value={editUrl} onChange={ev => setEditUrl(ev.target.value)} className={inputCls} />
                                                    </div>
                                                </div>
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <button onClick={() => saveEdit(e.id)}
                                                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold bg-emerald-500 hover:bg-emerald-600 text-white transition-all active:scale-95">
                                                        <Check size={14} /> Save
                                                    </button>
                                                </div>
                                            </div>
